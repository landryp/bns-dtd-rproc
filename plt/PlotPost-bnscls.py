...
LIKEDIR = '/home/philippe.landry/bns-dtd-rproc/dat/SAGA_bnscls/'
LIKEDIR_DISK = '/home/philippe.landry/bns-dtd-rproc/dat/Battistini_bnscls/'
LIKEPATH = 'SAGA_MP_5M.csv'
LIKEPATH_DISK = 'Battistini16_disk_5M.csv'

POPDIR = '/home/philippe.landry/bns-dtd-rproc/dat/'
POPPATH = 'EuFe_bnscls-10.h5'
...
ax.hist(list(alphas_disk)+alphas_disk_reflect,density=True,bins=np.arange(-2.9,-0.9,0.1),color=sns.color_palette()[0],alpha=0.2)
...
for tmin, tmin_disk, tmin_halo in zip(tmins,tmins_disk,tmins_halo): # reflect across hard prior bounds
    ...
    if tmin_disk < 0.015:
        tmins_disk_reflect += [2*(0.01)-tmin_disk]
          
    elif tmin_disk > 0.9:
        tmins_disk_reflect += [2*(2.01)-tmin_disk]
...
ax.hist(list(np.log10(tmins_disk))+list(np.log10(tmins_disk_reflect)),density=True,bins=np.arange(-1.9,0.1,0.1),color=sns.color_palette()[0],alpha=0.2)
...
#plt.tight_layout()
plt.subplots_adjust(bottom=0.2)
plt.savefig('ratemej_bnscls.png')
