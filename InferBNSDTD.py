...
parser.add_argument('-N','--nstep',default=20)
...
NSTP = int(args.nstep) # number of subdivisions per time step in chemical evolution integration
if args.multi: 
...
if args.verbose: print('building priors')
...
if args.verbose: print('loading observations')
...
if args.verbose: print('setting up emcee')
...
if args.verbose: print('launching emcee run')
...
if args.multi:        
    with multiprocessing.Pool(NWALK) as pool:
        ...
        elapsed = end - start
        if args.verbose: print("multiprocessing took {0:.1f} min".format(elapsed/60.))
else:
    if args.verbose: print('running')
    ...
    elapsed = end - start
    if args.verbose: print("emcee took {0:.1f} min".format(elapsed/60.))
...
if args.verbose: print('computing chemical evolution')
...
for i,(alpha,log10tmin,ratemej,X0) in enumerate(flat_samples):
